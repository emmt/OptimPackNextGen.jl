...
immutable CompactRegCauchy <: AbstractCost
    ...
    function CompactRegCauchy(_a::Cdouble,
        ...
        _a > 0.0 || error("radius must be strictly positive")
        ...
        for i2 in 1:_imgDim[2]
            i2f2 = _pixDim[2]*Float(i2) - _center[2]
            i2f2 *= i2f2
            for i1 in 1:_imgDim[1]
                i1f=_pixDim[1]*Float(i1) - _center[1]
                _w[i1,i2] = (1.0 + 2.0*(i1f*i1f + i2f2)/a2)
            end
        end
        new(_a,_center,_pixDim,_imgDim,_w/sum(_w))
    end
end
