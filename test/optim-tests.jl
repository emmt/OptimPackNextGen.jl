...
function rosenbrock_fgAD!(x::Vector{T}) where {T<:Real}
  const c1 = T(1)
  const c2 = T(2)
  const c10 = T(10)
  const c200 = T(200)
...
for (T, prec) in ((Float64, "double"), (Float32, "single"))
    ...
    # warmup).
    @printf("\nTesting L-BFGS in %s precision and with Armijo's line search\n", prec)
    x = vmlmb(rosenbrock_fgAD!, x0; verb=true, fmin=0, lnsrch=armijo)
    #@test_approx_eq_eps x ones(T,n) 1e-3
    @printf("\nTesting L-BFGS in %s precision and with Moré & Toraldo line search\n", prec)
    x = vmlmb(rosenbrock_fgAD!, x0; verb=true, fmin=0, lnsrch=moretoraldo)
    #@test_approx_eq_eps x ones(T,n) 1e-3
    @printf("\nTesting L-BFGS in %s precision and with Moré & Thuente line search\n", prec)
    x = vmlmb(rosenbrock_fgAD!, x0; verb=true, fmin=0, lnsrch=morethuente)
    #@test_approx_eq_eps x1 ones(T,n) 1e-3
    @printf("\nTesting BLMVM in %s precision and with Armijo's line search\n", prec)
    x = vmlmb(rosenbrock_fgAD!, x0; verb=true, fmin=0, lower=0, lnsrch=armijo, blmvm=true)
    #@test_approx_eq_eps x ones(T,n) 1e-3
    @printf("\nTesting BLMVM in %s precision and with Moré & Toraldo line search\n", prec)
    x = vmlmb(rosenbrock_fgAD!, x0; verb=true, fmin=0, lower=0, lnsrch=moretoraldo, blmvm=true)
    #@test_approx_eq_eps x ones(T,n) 1e-3
    @printf("\nTesting VMLMB in %s precision and with Armijo's line search\n", prec)
    x = vmlmb(rosenbrock_fgAD!, x0; verb=true, fmin=0, lower=0, lnsrch=armijo)
    #@test_approx_eq_eps x ones(T,n) 1e-3
    @printf("\nTesting VMLMB in %s precision and with Moré & Toraldo line search\n", prec)
    x = vmlmb(rosenbrock_fgAD!, x0; verb=true, fmin=0, lower=0, lnsrch=moretoraldo)
    #@test_approx_eq_eps x ones(T,n) 1e-3

    # Then run tests for timings.
    @printf("\nTesting L-BFGS in %s precision and with Armijo's line search\n", prec)
    @time x = vmlmb(rosenbrock_fgAD!, x0; verb=false, fmin=0, lnsrch=armijo)
    @printf("\nTesting L-BFGS in %s precision and with Moré & Toraldo line search\n", prec)
    @time x = vmlmb(rosenbrock_fgAD!, x0; verb=false, fmin=0, lnsrch=moretoraldo)
    @printf("\nTesting L-BFGS in %s precision and with Moré & Thuente line search\n", prec)
    @time x = vmlmb(rosenbrock_fgAD!, x0; verb=false, fmin=0, lnsrch=morethuente)


    @printf("\nTesting BLMVM in %s precision and with Armijo's line search\n", prec)
    @time x = vmlmb(rosenbrock_fgAD!, x0; verb=false, fmin=0, lower=0, lnsrch=armijo, blmvm=true)
    @printf("\nTesting BLMVM in %s precision and with Moré & Toraldo line search\n", prec)
    @time x = vmlmb(rosenbrock_fgAD!, x0; verb=false, fmin=0, lower=0, lnsrch=moretoraldo, blmvm=true)

    @printf("\nTesting VMLMB in %s precision and with Armijo's line search\n", prec)
    @time x = vmlmb(rosenbrock_fgAD!, x0; verb=false, fmin=0, lower=0, lnsrch=armijo)
    @printf("\nTesting VMLMB in %s precision and with Moré & Toraldo line search\n", prec)
    @time x = vmlmb(rosenbrock_fgAD!, x0; verb=false, fmin=0, lower=0, lnsrch=moretoraldo)
