...
get_counter() = cnt[]
increment_counter() = (cnt[] += 1; nothing)
reset_counter() = (cnt[] = 0; nothing)
...
const fzero_test1 = TestFunc(
    "fzero_test1",
    (x) -> (increment_counter(); 1/(x - 3) - 6),
    3, 4, 19/6)

const fzero_test2 = TestFunc(
    "fzero_test2",
    (x) -> (increment_counter(); exp(-x) - cos(x)),
    -4.7, 1.1, 0.0)

const fzero_test3 = TestFunc(
    "fzero_test3",
    (x) -> (increment_counter(); log(x^2 + x + 2) - x + 1),
    -1.8, 5.7, 4.152590736757158)

const fzero_test4 = TestFunc(
    "fzero_test4",
    (x) -> (increment_counter(); sin(x)^2 - x^2 + 1),
    -1.3, 7.2, 1.4044916482153411)

const fzero_test5 = TestFunc(
    "fzero_test5",
    (x) -> (increment_counter(); exp(-x^2 + x + 2) - cos(x + 1) + x^3 + 1),
    -2.1, 2.7, -1.0)

const fzero_test6 = TestFunc(
    "fzero_test6",
    (x) -> (increment_counter(); x^11 + x + 1),
    -2.7, 3.2, -0.844397528792023)

const fzero_test7 = TestFunc(
    "fzero_test7",
    (x) -> (increment_counter(); (x - 2)*(x^10 + x + 1)*exp(x - 1)),
    -2.7, 5.2, 2.0)

# Like f6 but with units.
const fzero_test8 = TestFunc(
    "fzero_test8",
    (x) -> (increment_counter(); x^11 + x*u"cm^10" + 1u"cm^11"),
...
const fzero_gsl1 = TestFunc(
    "fzero_gsl1",
    (x) -> (increment_counter(); x^20 - 1),
    0.1, 2.0, 1.0)

const fzero_gsl2 = TestFunc(
    "fzero_gsl2",
    (x) -> (increment_counter(); sqrt(abs(x))*sign(x)),
...
const fzero_gsl3 = TestFunc(
    "fzero_gsl3",
    (x) -> (increment_counter(); x^2 - fp(x, 1e-8)),
    0, 1, 1e-4)

const fzero_gsl4 = TestFunc(
    "fzero_gsl4",
    (x) -> (increment_counter(); x*exp(-x)),
    # NOTE: Since the solution is 0, setting atol to a sensible value is
    #       mandatory.
    -1.0, 5.0, 0.0)
